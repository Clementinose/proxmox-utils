...
clear
echo -e "${CYAN}╔═══════════════════════════════════════╗${RESET}"
echo -e "${CYAN}║${WHITE}      Clementinose Terminal Menu       ${CYAN}║${RESET}"
echo -e "${CYAN}╚═══════════════════════════════════════╝${RESET}"
echo ""
...
run_script() {
    echo -e "${BLUE}▶ Kör $1...${RESET}"
    bash <(curl -fsSL "$2") "$PUBKEY"
    echo ""
    read -p "Tryck Enter för att återgå till menyn..."
}
...
case "$choice" in
  ...
  7) echo -e "${RED}👋 Hej då${RESET}" ; exit 0 ;;
